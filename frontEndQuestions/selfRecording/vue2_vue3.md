...
1. **默认插槽（匿名插槽）** ：无法指定插入位置，只在第一个slot中插入
   1. 子组件使用：```<slot></slot>```
   2. 父组件中对子组件插入：
         ```
         <children>
         ...
         ```
2. **具名插槽** ：可以指定插入哪个slot中
   1. 子组件使用：```<slot name="xx"></slot>```
   2. 父组件中对子组件插入：
         ```
         <children>
            <template v-slot:xx >插入内容</template>
            //缩写
            <template #xx>插入内容</template>
         </children>
         ```
3. **作用域插槽** ：作用域插槽就是实现在子组件**自行决定**自己要显示什么内容
   1. 子组件使用：
         ```
            <slot name="xx" :obj="obj"></slot>//obj为子组件想告知父组件的属性
         ```
   3. 父组件中对子组件插入：
         ```
         <children>
            <template v-slot:xx >{{xx.obj.变量名}}</template>
         </children>
         ```
